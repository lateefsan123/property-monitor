...
- Templates (revision 5): the existing Transaction update template's Preview sheet shows the full attached broker card, including the user's brother's portrait, above the sample message. No template or attachment changes were saved.
- Updated exports: `exports/repeat-ai-v5-05-assistant-draft.png` and `exports/repeat-ai-v5-06-templates-draft.png`. Earlier revision 4 files remain for comparison. These are still mobile web drafts pending matching native iPhone captures.
