...
t=json.loads((ROOT/'video/src/workflow/final-timeline.json').read_text())
...
x=E.Element('xmeml',version='5');seq=tag(x,'sequence',id='repeat-final');tag(seq,'name','Repeat AI - Final ElevenLabs');tag(seq,'duration',t['frames']);rate(seq)
...
for i,r in enumerate(rows):
 ...
 if i==0:
  tag(f,'name','repeat-ai-final-master.mp4');tag(f,'pathurl',(OUT/'repeat-ai-final-master.mp4').as_uri());rate(f);tag(f,'duration',t['frames']);sample(tag(tag(tag(f,'media'),'video'),'samplecharacteristics'))
...
for ch in [1,2]:
 ...
 if ch==1:
  p=ROOT/'video/assets/accurate/public/workflow/final/mix.wav';tag(f,'name',p.name);tag(f,'pathurl',p.as_uri());rate(f);tag(f,'duration',t['frames']);am=tag(tag(f,'media'),'audio');tag(am,'channelcount',2);a=tag(am,'samplecharacteristics');tag(a,'depth',24);tag(a,'samplerate',48000)
 st=tag(c,'sourcetrack');tag(st,'mediatype','audio');tag(st,'trackindex',ch)
E.indent(x)
E.ElementTree(x).write(OUT/'repeat-ai-final-resolve.xml',encoding='utf-8',xml_declaration=True)
