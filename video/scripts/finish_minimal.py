...
OUT=ROOT/'video/review/minimal'
ASSETS=ROOT/'video/assets/minimal/public/video/repeat-ai-minimal'
...
def main():
 ...
 run(['ffmpeg','-v','error','-y','-i',str(OUT/'repeat-ai-minimal-master.mp4'),'-i',str(ASSETS/'mix.wav'),'-i',str(ASSETS/'captions.srt'),'-map','0:v:0','-map','1:a:0','-map','2:0','-c:v','copy','-c:a','aac','-b:a','320k','-c:s','mov_text','-t','67','-metadata:s:s:0','language=eng','-disposition:s:0','0','-metadata','title=Repeat AI - The next conversation','-metadata','comment=Illustrated product demonstration using fictional seller details and example asking prices.','-metadata','copyright='+CREDIT,'-movflags','+faststart',str(movie)])
 ...
 assert v['width']==1920 and v['height']==1080 and v['r_frame_rate']=='30/1' and int(v['nb_frames'])==2010
 assert a['channels']==2
 assert abs(float(probe['format']['duration'])-67)<.05
 ...
 report=dict(file=movie.name,duration=67,width=1920,height=1080,fps=30,frames=2010,full_decode='pass',black_frames='none',optional_captions='English, off by default',loudness=loudness,music_credit=CREDIT,sha256=hashlib.sha256(movie.read_bytes()).hexdigest(),bytes=movie.stat().st_size)
 (OUT/'verification.json').write_text(json.dumps(report,indent=2)+'\n')
 # Inspect four successive states for each key action, not only finished scene layouts.
 for name,start in [('import',7.1),('price',15.5),('voice',25),('typing',31.5),('schedule',42.7),('responsive',57.5)]:
