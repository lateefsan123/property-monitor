...
import {AbsoluteFill, Audio, Freeze, Sequence, useCurrentFrame, interpolate} from 'remotion';
...
function Outgoing({id,closed=false}:{id:string;closed?:boolean}){
...
function Navigation({id,frames}:{id:string;frames:number}){
 ...
 return <AbsoluteFill className="app-shell">
  {id==='import'&&f>=menuEnd?<ImportLanding/>:<Outgoing id={id} closed={closed}/>}
...
function Scene({row}:{row:any}){
 ...
 if(f<nav)return <Navigation id={row.id} frames={nav}/>;
 const local=f-nav;
...
export function FinalFilm(){return <AbsoluteFill className="accurate-film workflow-film" data-theme="dark"><Audio src={asset('workflow/final/mix.wav')}/>{timeline.scenes.map(row=><Sequence key={row.id} from={row.from} durationInFrames={row.frames}><Scene row={row}/></Sequence>)}<Sequence from={timeline.closeFrom} durationInFrames={90}><Close/><div style={{position:'absolute',bottom:310,width:'100%',textAlign:'center',fontSize:28,color:'#c7c2b9'}}>repeatai.org</div></Sequence></AbsoluteFill>}
