...
function Schedule(){const f=useCurrentFrame()*26/24;const active=slots.find(s=>f>=s.start&&f<s.end);const enabled=f>=55;const sourceIndex=active?(active.select.findIndex(t=>f<t+9)<0?active.buildings.length-1:active.select.findIndex(t=>f<t+9)):0;const sourceName=active?.buildings[sourceIndex]||'';const switching=active?.select.slice(0,-1).find(t=>f>=t+5&&f<t+18);const saved=f>=630;
...
function Integrations(){const f=useCurrentFrame();const icons=[['sheets.png','Google Sheets'],['excel.svg','Excel'],['gmail.png','Gmail'],['outlook.svg','Outlook'],['calendar.png','Google Calendar']];return <AbsoluteFill style={{justifyContent:'center',alignItems:'center'}}><h1 style={{fontSize:66,marginTop:-95}}>Integrations</h1><div style={{display:'flex',gap:90,marginTop:80}}>{icons.map(([file,label],i)=><div key={file} style={{width:170,textAlign:'center',opacity:tween(f,i*13,i*13+20),transform:`translateY(${tween(f,i*13,i*13+25,22,0)}px)`}}><Img src={asset('workflow/'+file)} style={{height:96,width:96,objectFit:'contain'}}/><div style={{fontSize:23,marginTop:27,color:'#d7d0c5'}}>{label}</div></div>)}</div></AbsoluteFill>}
