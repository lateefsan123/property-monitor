...
import {AudioLines, Square, Plus, X, ArrowUp} from 'lucide-react';
...
export function Assistant({contextual=false}:{contextual?:boolean}){
 ...
 <div className="repeat-assistant is-open" style={{position:'absolute',left:contextual?tween(f,0,20,1920,1299):1040,top:contextual?0:65,height:contextual?800:undefined,right:'auto',bottom:'auto',opacity:contextual?tween(f,0,7):1,transform:'scale(1.35)',transformOrigin:'top left'}}><section className="assistant-panel"><header className="assistant-header"><span>Repeat AI</span><div className="assistant-header-actions"><button aria-label="New chat"><Plus size={26}/></button><button aria-label="Close and end conversation"><X size={24}/></button></div></header>
 <div className={`assistant-body${!sent?' is-empty':''}`} ref={body}>{!sent?<><Orb frame={f}/><h2>What can I help with?</h2></>:<><div className="assistant-chat-log"><p className="assistant-chat-message is-user">{request}</p>{!review?<p>Thinking…</p>:<p className="assistant-chat-message is-assistant">I’ve drafted a friendly follow-up using the latest sale. Have a look before saving it.</p>}</div>{review&&<div className="assistant-approval" style={{opacity:tween(f,350,360)}}><h3>Create template “Recent sale follow-up”?</h3><p>{'Hi {{name}}, a home has just sold in {{building}}.\n\n{{transactions}}\n\nWould you like me to check what your property could sell for?\n\nSave as a new template: Recent sale follow-up.'}</p><button>Confirm change</button><button>Discard</button></div>}</>}
 </div><footer className="assistant-footer"><div className={`assistant-composer${f>=35&&!sent?' wf-is-typing':''}`}><TypedField multiline value={draft} active={f>=35&&!sent} placeholder="Message Repeat" className="wf-assistant-draft" style={{height:draft.length>40?66:44}}/>{!sent&&draft?<button aria-label="Send message"><ArrowUp size={22}/></button>:sent&&!review?<button aria-label="Stop response"><Square size={20}/></button>:null}</div><button className="assistant-voice-toggle" aria-label="Start voice conversation" disabled={sent}><AudioLines size={24}/></button></footer>
 </section></div>
 {f>=295&&f<329&&<Cursor x={tween(f,295,315,1330,contextual?1788:1529)} y={contextual?1029:815} click={Math.max(0,1-Math.abs(f-320)/6)}/>}
 </AbsoluteFill>;
}
