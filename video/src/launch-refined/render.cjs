...
const root=path.resolve(__dirname,'../../..'),out=path.join(root,'video/review/launch-refined');
async function main(){
 fs.mkdirSync(out,{recursive:true});
 const serveUrl=await bundle({entryPoint:path.join(__dirname,'index.tsx'),publicDir:path.join(root,'video/assets/launch-refined/public'),outDir:path.join(out,'bundle'),webpackOverride:c=>({...c,module:{...c.module,rules:[...(c.module?.rules||[]),{test:/\.m?js$/,resolve:{fullySpecified:false}}]},resolve:{...c.resolve,alias:{...c.resolve?.alias,react:path.join(root,'video/node_modules/react'),'react-dom':path.join(root,'video/node_modules/react-dom')}}})});
 const browser=await openBrowser('chrome');
 try{
  const composition=await selectComposition({serveUrl,id:'RepeatAIRefined',puppeteerInstance:browser});
  ...
  let last=-1;await renderMedia({serveUrl,composition,puppeteerInstance:browser,codec:'h264',outputLocation:path.join(out,'repeat-ai-refined-master.mp4'),crf:17,pixelFormat:'yuv420p',audioBitrate:'320k',concurrency:3,onProgress:p=>{const pc=Math.floor(p.progress*20)*5;if(pc!==last){last=pc;console.log('RENDER',pc+'%')}}});
