...
export function createHomeInsightServices(
  supabase,
  { sanitizeChangeItem, sanitizeListingHistoryEntry },
) {
  ...
  async function fetchWhatsAppMessageActivity(userId, days = 14) {
    if (!userId) return [];
  ...
  function buildDailyMessageSeries(messageRows, days = 14) {
  ...
  async function fetchListingPriceDrops(userId) {
    if (!userId) return [];
    ...
    const historyEntries =
      data?.listing_history && typeof data.listing_history === "object"
    ...
    for (const [key, rawEntry] of historyEntries) {
      ...
      const windowDrops = (entry.priceHistory || [])
        .filter(
          (event) =>
            event?.type === "price_drop" && toEventTime(event) >= cutoff,
        )
      ...
      if (!item || !Number.isFinite(item.priceDelta) || item.priceDelta >= 0)
        continue;
    ...
    const changeItems = Array.isArray(data?.change_items)
      ? data.change_items
      : [];
