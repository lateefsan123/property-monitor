...
} from "../src/features/seller-signal/insight-utils";
import {
  cleanBuildingName,
...
export function createLeadInsightServices(supabase) {
  ...
  async function fetchAvailableBuildingKeys(buildingKeys) {
    ...
    if (!error) {
      return new Set(
        (data || []).map((row) => row.building_key).filter(Boolean),
      );
    }
  ...
  async function fetchAvailableMarketBuildingKeys(buildingKeys) {
  ...
  async function fetchBuildingKeysWithTransactionsOn(buildingKeys, dateKey) {
    const uniqueKeys = [...new Set(buildingKeys)].filter(Boolean);
  ...
  async function fetchTransactionsForKeys(buildingKeys) {
    ...
    for (
      let index = 0;
      index < buildingKeys.length;
      index += TRANSACTION_FETCH_BATCH_SIZE
    ) {
      const batch = buildingKeys.slice(
        index,
        index + TRANSACTION_FETCH_BATCH_SIZE,
      );
      const results = await Promise.all(
  ...
  async function fetchBuildingMarketData(buildingKeys) {
    const uniqueKeys = [...new Set(buildingKeys)].filter(Boolean);
    if (!uniqueKeys.length) {
      return { buildingLookup: {}, transactionsByBuilding: {} };
    }

    const [{ data: buildingRows, error: buildingError }, availableKeySet] =
      await Promise.all([
        supabase
          .from("buildings")
          .select("key, search_name, location_name")
          .in("key", uniqueKeys),
    ...
    const transactionsByBuilding = await fetchTransactionsForKeys([
      ...availableKeySet,
    ]);

    const buildingLookup = {};
    for (const building of buildingRows || [])
      buildingLookup[building.key] = building;

    return { buildingLookup, transactionsByBuilding };
  }

  function getMissingFallbackBuildingNames(targets, transactionsByBuilding) {
    const missingNames = new Set();

    for (const lead of targets || []) {
      const keys = getBuildingKeyVariants(lead.building);
      if (!keys.length) continue;
      const { transactions } = findTransactionsForKeys(
        keys,
        transactionsByBuilding || {},
      );
  ...
  function computeLeadInsights(
    ...
    for (const lead of targets || []) {
      ...
      const fallbackMatch = findTransactionsForKeys(
        keys,
        fallbackTransactionsByBuilding,
      );
      const matchedKey =
        cachedMatch.key ||
        fallbackMatch.key ||
        keys.find((key) => buildingLookup[key]) ||
        keys[0];
      const building = matchedKey ? buildingLookup[matchedKey] : null;
      const allTransactions = cachedMatch.transactions.length
        ? cachedMatch.transactions
        : fallbackMatch.transactions;
      const locationName =
        building?.search_name ||
      ...
      const allTransactionDates = filteredTransactions
