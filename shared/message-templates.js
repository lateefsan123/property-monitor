...
export function createMessageTemplateServices(
  ...
  const MESSAGE_TEMPLATE_IMAGE_BUCKET = "seller-signal-template-images";
  const MESSAGE_TEMPLATE_IMAGE_MAX_BYTES = 5 * 1024 * 1024;
  const MESSAGE_TEMPLATE_IMAGE_TYPES = [
    "image/jpeg",
    "image/png",
    "image/webp",
  ];
  ...
  async function addTemplateImagePreviews(templates = []) {
    const paths = [
      ...new Set(
        templates.map((template) => template.image_path).filter(Boolean),
      ),
    ];
    if (!paths.length)
      return templates.map((template) => ({ ...template, image_url: null }));
  ...
  async function uploadTemplateImage(userId, file) {
    ...
    const path = `${userId}/${randomUUID()}.${getImageExtension(file)}`;
    const { error } = await supabase.storage
      .from(MESSAGE_TEMPLATE_IMAGE_BUCKET)
      .upload(path, file.uploadBody ?? file, {
        cacheControl: "3600",
  ...
  async function saveMessageTemplate({
    ...
    if (imagePath && imagePath !== nextImagePath) {
      await removeTemplateImage(imagePath).catch((cleanupError) => {
        console.warn(
          "Could not remove the replaced template image",
          cleanupError,
        );
      });
    }
  ...
  async function setDefaultMessageTemplate({ id, userId }) {
  ...
  async function deleteMessageTemplate({ id, userId }) {
    const { data, error } = await supabase
      .from("seller_signal_message_templates")
      .delete()
      .eq("id", id)
    ...
    if (data?.image_path) {
      await removeTemplateImage(data.image_path).catch((cleanupError) => {
        console.warn(
          "Could not remove the deleted template image",
          cleanupError,
        );
      });
    }
